...
#include "loilo.h"
#include <unistd.h>
#include "utils/android_bitmap_lock.h"
#include "external/utils/Mutex.h"
#include "external/pdfium/fpdfsdk/include/fpdfview.h"
#include "external/pdfium/fpdfsdk/include/fsdk_rendercontext.h"

extern "C" {

JNIEXPORT jlong JNICALL Java_tv_loilo_pdf_PdfRendererCompat_nativeCreate(JNIEnv *env, jobject thiz, jint fd, jlong size);

JNIEXPORT jint JNICALL Java_tv_loilo_pdf_PdfRendererCompat_nativeGetPageCount(JNIEnv *env, jobject thiz, jlong documentPtr);

JNIEXPORT void JNICALL Java_tv_loilo_pdf_PdfRendererCompat_nativeClose(JNIEnv* env, jobject thiz, jlong documentPtr);

JNIEXPORT jlong JNICALL Java_tv_loilo_pdf_PdfRendererCompat_nativeOpenPageAndGetSize(JNIEnv *env, jobject thiz, jlong documentPtr, jint pageIndex, jdoubleArray outSize);

JNIEXPORT void JNICALL Java_tv_loilo_pdf_PdfRendererCompat_nativeRenderPage(JNIEnv *env, jobject thiz, jlong documentPtr, jlong pagePtr, jobject jbitmap, jint destLeft, jint destTop, jint destRight, jint destBottom, jfloatArray matrix, jint renderMode);

JNIEXPORT jboolean JNICALL Java_tv_loilo_pdf_PdfRendererCompat_nativeScaleForPrinting(JNIEnv *env, jobject thiz, jlong documentPtr);

JNIEXPORT void JNICALL Java_tv_loilo_pdf_PdfRendererCompat_nativeClosePage(JNIEnv * env,jobject thiz, jlong pagePtr);
}

static android::Mutex sLibraryLock;
...
static void initializeLibraryIfNeeded(){
    android::Mutex::Autolock lock(sLibraryLock);
        ...
        if (!document) {
            ...
            switch (error) {
                case FPDF_ERR_PASSWORD:{
                    ...
                    loilo::post_throw(env, "java/lang/SecurityException", "Failed to create document. error = FPDF_ERR_PASSWORD");
                    return 0;
                }
                case FPDF_ERR_SECURITY: {
                    destroyLibraryIfNeeded();
                    loilo::post_throw(env, "java/lang/SecurityException", "Failed to create document. error = FPDF_ERR_SECURITY");
                    return 0;
                }
                default: {
                    destroyLibraryIfNeeded();
...
JNIEXPORT void JNICALL Java_tv_loilo_pdf_PdfRendererCompat_nativeRenderPage(JNIEnv * env, jobject thiz, jlong documentPtr, jlong pagePtr, jobject jbitmap, jint destLeft, jint destTop, jint destRight, jint destBottom, jfloatArray matrix, jint renderMode) {
    loilo::run(env, [=]() {
        ...
        if (info.format != ANDROID_BITMAP_FORMAT_RGBA_8888) {
            throw std::invalid_argument("Invalid format. only ANDROID_BITMAP_FORMAT_RGBA_8888 is accepted.");
        }

        loilo::android_bitmap_lock lock(env, jbitmap);
        {
            FPDF_PAGE page = reinterpret_cast<FPDF_PAGE>(pagePtr);
            FPDF_BITMAP bitmap = FPDFBitmap_CreateEx(info.width, info.height, FPDFBitmap_BGRA, lock.pixels(), info.stride);
            ...
            renderPageBitmap(bitmap, page, destLeft, destTop, destRight, destBottom, transform, renderFlags);
            if(transform){
                delete[] transform;
            }
        }
